...
class Game extends React.Component {
  async componentDidMount() {
    ...
    this.setState({
      title_arr: tem_title_arr,
      description_arr: tem_description_arr,
    })
  }
  state = {
    isShown: false,
  ...
  check_answers() {
    ...
    for (var j = 0; j < this.state.data.length; j++) {
      if (
        String.fromCharCode(
          tem_description_arr.indexOf(tem_title_arr[j]) + 65
      ...
      <Pane display="flex" justifyContent="space-between">
        ...
        >
          <Heading
            ...
            marginTop={50}
          >
            Hướng dẫn: Ghép các từ với nghĩa phù hợp rồi điền vào ô trống
          </Heading>
          <Pane>
            {!this.state.data
              ? 'loading'
              : this.state.title_arr.map((item, index) => (
                  <Pane
                    display="flex"
                  ...
                  >
                    ...
                    <Pane
                      ...
                      background="#DDEBF7"
                    >
                      <Heading size={400}>{`${String.fromCharCode(
                        index + 65
                      )}. ${
                        this.state.data[this.state.description_arr[index]]
                          .description
                      }`}</Heading>
                    </Pane>
                  </Pane>
          ...
          <Pane width="80%" display="flex">
            {!this.state.data
              ? 'loading'
              : this.state.title_arr.map((title, index) => (
                  <Pane
                    key={index}
                    marginBottom={20}
                    marginTop={20}
                    ...
                    paddingLeft={20}
                  >
                    <Heading size={500} fontWeight={600}>{`Câu số ${
                      index + 1
                    }:`}</Heading>
                    <TextInput
                      borderRadius={5}
